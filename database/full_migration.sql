...
-- =============================================
-- TABLE: task_groups
-- Purpose: Hierarchical task grouping (WBS structure, max 2 levels)
-- =============================================
CREATE TABLE IF NOT EXISTS public.task_groups (
    ...
    project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
    parent_group_id UUID REFERENCES public.task_groups(id) ON DELETE CASCADE,
    group_type group_type NOT NULL DEFAULT 'group',
    ...
    CONSTRAINT task_groups_name_check CHECK (char_length(name) >= 1),
    CONSTRAINT task_groups_no_deep_nesting CHECK (
        group_type = 'group' OR parent_group_id IS NOT NULL
    )
);

CREATE INDEX IF NOT EXISTS idx_task_groups_project ON public.task_groups(project_id);
CREATE INDEX IF NOT EXISTS idx_task_groups_parent ON public.task_groups(parent_group_id);
CREATE INDEX IF NOT EXISTS idx_task_groups_tenant ON public.task_groups(tenant_id);
CREATE INDEX IF NOT EXISTS idx_task_groups_tg_id ON public.task_groups(tg_id) WHERE tg_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_task_groups_sort ON public.task_groups(project_id, parent_group_id, sort_order);

COMMENT ON TABLE public.task_groups IS '태스크 그룹 (WBS 계층 구조, 최대 2레벨: 그룹 → 서브그룹)';
COMMENT ON COLUMN public.task_groups.wbs IS 'Work Breakdown Structure 코드 (예: 1, 1.1, 2.3)';
COMMENT ON COLUMN public.task_groups.parent_group_id IS 'NULL이면 최상위 그룹, 값이 있으면 서브그룹';

-- =============================================
-- TABLE: tasks
-- Purpose: Individual tasks (Gantt chart work items)
-- =============================================
...
CREATE INDEX IF NOT EXISTS idx_tasks_tg_id ON public.tasks(tg_id) WHERE tg_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_tasks_dates ON public.tasks(project_id, start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_tasks_sort ON public.tasks(group_id, sort_order);
CREATE INDEX IF NOT EXISTS idx_tasks_status ON public.tasks(project_id, percent_complete);

COMMENT ON TABLE public.tasks IS '개별 태스크 (간트차트의 최소 작업 단위)';
COMMENT ON COLUMN public.tasks.slack IS '크리티컬 패스 분석 시 여유 시간 (일 단위)';
COMMENT ON COLUMN public.tasks.is_milestone IS '마일스톤 태스크 (기간 0일, 주요 이정표)';
COMMENT ON COLUMN public.tasks.days IS '작업 기간 (근무일 기준)';
COMMENT ON COLUMN public.tasks.custom_fields IS 'JSONB: 사용자 정의 필드 (확장성)';

-- =============================================
-- TABLE: task_assignees
-- Purpose: Task-user assignments (resource allocation)
-- =============================================
...
COMMENT ON TABLE public.task_assignees IS '태스크-담당자 매핑 (리소스 할당)';
...
-- Purpose: Task relationship links (Gantt chart arrows)
-- =============================================
...
COMMENT ON TABLE public.task_dependencies IS '태스크 간 의존성 관계 (간트차트 화살표)';
...
-- =============================================
-- TABLE: comments
-- Purpose: Comments on projects/groups/tasks (polymorphic)
-- =============================================
...
COMMENT ON TABLE public.comments IS '프로젝트/그룹/태스크에 대한 코멘트 (다형성 관계)';
COMMENT ON COLUMN public.comments.target_id IS 'target_type에 따라 projects, task_groups, tasks의 id 참조';
...
CREATE INDEX IF NOT EXISTS idx_checklist_sort ON public.checklist_items(task_id, sort_order);

COMMENT ON TABLE public.checklist_items IS '태스크 체크리스트 항목';
...
COMMENT ON COLUMN public.activity_log.target_type IS 'project, task, group, comment, document 등';
...
CREATE POLICY "Editors can create task groups"
    ON public.task_groups
    FOR INSERT
    TO authenticated
    WITH CHECK (
...
CREATE POLICY "Editors can update task groups"
    ON public.task_groups
    FOR UPDATE
    TO authenticated
    USING (
...
CREATE POLICY "Editors can delete task groups"
    ON public.task_groups
    FOR DELETE
    TO authenticated
    USING (
...
COMMENT ON TRIGGER update_task_groups_updated_at ON public.task_groups IS 'task_groups 수정 시 updated_at 자동 갱신';
COMMENT ON TRIGGER update_tasks_updated_at ON public.tasks IS 'tasks 수정 시 updated_at 자동 갱신';
