...
import type { TGProject, TGAccess, TGProjectChild, TGBoard } from '../../types/teamgantt.js';
...
export async function listProjects(client: ApiClient, logger: Logger): Promise<TGProject[]> {
  ...
  for (const status of statuses) {
    ...
    while (true) {
      const response = await client.get<Record<string, unknown>>('/projects', { status, page });
...
export async function getProject(
  ...
  const response = await client.get<Record<string, unknown>>(`/projects/${projectId}`);
  const project = (response.data?.data ?? response.data) as TGProject;
...
export async function getProjectChildren(
  ...
  const response = await client.get<Record<string, unknown>>(`/projects/${projectId}/children`);
...
export async function getProjectAccesses(
  ...
  const response = await client.get<Record<string, unknown>>(`/projects/${projectId}/access`);
...
export async function getProjectBoards(
  ...
  const response = await client.get<Record<string, unknown>>(`/projects/${projectId}/boards`);
