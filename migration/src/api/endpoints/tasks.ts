...
export async function getTask(
  ...
  const response = await client.get<Record<string, unknown>>(`/tasks/${taskId}`);
...
export async function getTaskChecklist(
  ...
  try {
    const response = await client.get<Record<string, unknown>>(`/tasks/${taskId}/checklist`);
...
export async function getTaskDocuments(
  ...
  try {
    const response = await client.get<Record<string, unknown>>(`/tasks/${taskId}/documents`);
...
export async function getTaskComments(
  ...
  try {
    const response = await client.get<Record<string, unknown>>(`/tasks/${taskId}/comments`);
