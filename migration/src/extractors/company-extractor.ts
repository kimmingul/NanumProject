...
export async function runDiscovery(
  ...
  if (progress.isPhaseCompleted('discovery')) {
    ...
    const state = progress.getState() as Record<string, unknown>;
  ...
  const companies = userRecord.companies;
  const companyObj = userRecord.company as Record<string, unknown> | undefined;
  const companyId = String(
    currentUser.company_id ??
    companyObj?.id ??
    userRecord.companyId ??
    (Array.isArray(companies) && companies.length > 0 ? (companies[0] as Record<string, unknown>).id : '') ??
    '',
  );
  ...
  (progress.getState() as Record<string, unknown>).companyId = companyId;
