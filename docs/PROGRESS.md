...
> Note (2026-02): 초기에는 Auth0-like IDaaS(NanumAuth/CoreAuth)를 만들려다가,
> 이후 TeamGantt 데이터를 Supabase로 마이그레이션하면서 **NanumProject(프로젝트/간트 관리)** 기능이 메인으로 확장되었습니다.
> 인증 기능은 NanumProject의 필수 모듈로 유지하되, 제품/문서의 1순위는 NanumProject에 맞춥니다.
